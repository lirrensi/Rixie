# FILE: v1/export_html.py
# PURPOSE: Render legacy distillation outputs into the V1 HTML viewer using the bundled template.
# OWNS: V1 HTML template loading, chunk accordion assembly, and final HTML file generation.
...
from v1._paths import resolve_asset_path

TEMPLATE_PATH = resolve_asset_path("template.html")
...
def export_html(book_dir: Path, book_name: str, generate_final: bool = True) -> Path:
    ...
    data_textareas = ""
    for i, c in enumerate(chunks):
        data_textareas += f'<textarea id="data-chunk-{i}" hidden>{c["raw"]}</textarea>\n'
